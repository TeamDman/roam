...
impl roam_session::Caller for ShmConnectionHandle {
    async fn call_with_metadata<T: facet::Facet<'static> + Send>(
        &self,
        method_id: u64,
        args: &mut T,
        ...
        roam_session::Caller::call_with_metadata(&self.inner, method_id, args, metadata).await
    }
    ...
    D: ServiceDispatcher,
{
    ...
    let inner_handle = root_conn.handle.clone();

    let mut connections = HashMap::new();
    connections.insert(ConnectionId::ROOT, root_conn);

    // Create channel for incoming connection requests
    let (incoming_connections_tx, _incoming_connections_rx) = mpsc::channel(64);
    ...
    let driver = ShmDriver {
        ...
        diagnostic_state,
        var_slot_pool: var_slot_pool.clone(),
        local_peer_id,
    };
